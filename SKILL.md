# SKILL.md — Data Pipeline Failure RCA Agent
    ...
    - Pipeline/task identifiers and orchestrator metadata
- Execution logs, stack traces and query errors
- Recent code/config/schema/dependency changes
- Lineage and downstream dependencies
- Runtime metrics and historical baselines
...
- Validation tests and rollback plan
- Incident timeline
...
- `sql_explainer`
    ...
    - 1. `ingest_incident_context`
- 2. `normalize_logs_and_events`
- 3. `extract_error_signatures`
- 4. `generate_competing_hypotheses`
- 5. `collect_supporting_and_disconfirming_evidence`
- 6. `rank_root_causes`
- 7. `estimate_blast_radius`
- 8. `propose_remediation`
- 9. `generate_validation_and_rollback_plan`
    ...
    Maintain a provider-neutral interface in `src/pipeline_rca/providers/base.py`.
