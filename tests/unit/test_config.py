...
from pipeline_rca.config import load_config
...
def test_environment_override_is_typed() -> None:
    settings = load_config(
        DEFAULT_CONFIG,
        environment={
            "PIPELINE_RCA__PROVIDER__MAX_RETRIES": "5",
            "PIPELINE_RCA__TOOLS__ALLOW_MUTATIONS": "false",
        },
    )
...
def test_unsafe_approval_configuration_is_rejected() -> None:
    with pytest.raises(ValidationError, match="Mutations cannot be enabled"):
        load_config(
            DEFAULT_CONFIG,
            environment={
                "PIPELINE_RCA__TOOLS__ALLOW_MUTATIONS": "true",
                "PIPELINE_RCA__APPROVAL__MODE": "disabled",
            },
        )
