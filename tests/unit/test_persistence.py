...
from pipeline_rca.models import IncidentInput, RunState
from pipeline_rca.persistence import JsonRunStateRepository


def state() -> RunState:
    return RunState(
        run_id="safe-run_01",
        request=IncidentInput(case_id="case", title="Title", description="Description"),
    )
...
def test_json_repository_retries_transient_replace_lock(tmp_path: Path) -> None:
    ...
    with patch("pipeline_rca.persistence.os.replace", side_effect=flaky_replace):
